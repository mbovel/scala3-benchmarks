...
{ self =>
   ...
   */
  //protected def newBuilder: scala.collection.mutable.Builder[T, Repr] = newCombiner
  ...
  protected def sequentially[S, That <: Parallel](b: (Sequential => Sequential) @uncheckedVariance) = newCombiner.fromSequential(b(seq))
   ...
   *  thread safe access.
   */
  protected def combinerFactory: CombinerFactory[T @uncheckedVariance, Repr @uncheckedVariance] = {
  ...
  protected def combinerFactory[S, That](cbf: () => Combiner[S, That]) = {
    val combiner = cbf()
    combiner.combinerTaskSupport = tasksupport
  ...
  extends StrictSplitterCheckTask[R, Tp] {
    protected val pit: IterableSplitter[T]
    protected def newSubtask(p: IterableSplitter[T @uncheckedVariance]): Accessor[R, Tp]
  ...
  protected trait NonDivisibleTask[R, Tp] extends StrictSplitterCheckTask[R, Tp] {
    def shouldSplitFurther = false
    def split = throw new UnsupportedOperationException("Does not split.")
  }

  protected trait NonDivisible[R] extends NonDivisibleTask[R, NonDivisible[R]]

  protected abstract class Composite[FR, SR, R, First <: StrictSplitterCheckTask[FR, ?], Second <: StrictSplitterCheckTask[SR, ?]]
  ...
  protected abstract class SeqComposite[FR, SR, R, First <: StrictSplitterCheckTask[FR, ?], Second <: StrictSplitterCheckTask[SR, ?]]
  (f: First, s: Second)
  ...
  protected abstract class ParComposite[FR, SR, R, First <: StrictSplitterCheckTask[FR, ?], Second <: StrictSplitterCheckTask[SR, ?]]
  (f: First, s: Second)
  extends Composite[FR, SR, R, First, Second](f, s) {
    def leaf(prevr: Option[R]) = {
  ...
  protected abstract class ResultMapping[R, Tp, R1](val inner: StrictSplitterCheckTask[R, Tp])
  ...
  protected class Foreach[S](op: T => S, protected val pit: IterableSplitter[T @uncheckedVariance])
  extends Accessor[Unit, Foreach[S]] {
    @volatile var result: Unit = ()
    def leaf(prevr: Option[Unit]) = pit.foreach(op)
    protected def newSubtask(p: IterableSplitter[T @uncheckedVariance]) = new Foreach[S](op, p)
  }

  protected class Count(pred: T => Boolean, protected val pit: IterableSplitter[T @uncheckedVariance])
  extends Accessor[Int, Count] {
    ...
    protected def newSubtask(p: IterableSplitter[T @uncheckedVariance]) = new Count(pred, p)
    override def merge(that: Count) = result = result + that.result
    // override def toString = "CountTask(" + pittxt + ")"
  }

  protected class Reduce[U >: T](op: (U, U) => U, protected val pit: IterableSplitter[T @uncheckedVariance])
  extends Accessor[Option[U], Reduce[U]] {
    ...
    protected def newSubtask(p: IterableSplitter[T @uncheckedVariance]) = new Reduce(op, p)
  ...
  protected class Fold[U >: T](z: U, op: (U, U) => U, protected val pit: IterableSplitter[T @uncheckedVariance])
  extends Accessor[U, Fold[U]] {
    @volatile var result: U = null.asInstanceOf[U]
    def leaf(prevr: Option[U]) = result = pit.fold(z)(op)
    protected def newSubtask(p: IterableSplitter[T @uncheckedVariance]) = new Fold(z, op, p)
    override def merge(that: Fold[U]) = result = op(result, that.result)
  }

  protected class Aggregate[S](z: () => S, seqop: (S, T) => S, combop: (S, S) => S, protected val pit: IterableSplitter[T @uncheckedVariance])
  extends Accessor[S, Aggregate[S]] {
    ...
    protected def newSubtask(p: IterableSplitter[T @uncheckedVariance]) = new Aggregate(z, seqop, combop, p)
  ...
  protected class Map[S, That](f: T => S, cbf: CombinerFactory[S, That], protected val pit: IterableSplitter[T @uncheckedVariance])
  extends Transformer[Combiner[S, That], Map[S, That]] {
    ...
    protected def newSubtask(p: IterableSplitter[T @uncheckedVariance]) = new Map(f, cbf, p)
    override def merge(that: Map[S, That]) = result = result `combine` that.result
  }

  protected class Collect[S, That]
  (pf: PartialFunction[T, S], pbf: CombinerFactory[S, That], protected val pit: IterableSplitter[T @uncheckedVariance])
  extends Transformer[Combiner[S, That], Collect[S, That]] {
    ...
    protected def newSubtask(p: IterableSplitter[T @uncheckedVariance]) = new Collect(pf, pbf, p)
    override def merge(that: Collect[S, That]) = result = result `combine` that.result
  }

  protected class FlatMap[S, That]
  (f: T => IterableOnce[S], pbf: CombinerFactory[S, That], protected val pit: IterableSplitter[T @uncheckedVariance])
  extends Transformer[Combiner[S, That], FlatMap[S, That]] {
    @volatile var result: Combiner[S, That] = null
    def leaf(prev: Option[Combiner[S, That]]) = result = pit.flatmap2combiner(f, pbf())
    protected def newSubtask(p: IterableSplitter[T @uncheckedVariance]) = new FlatMap(f, pbf, p)
  ...
  protected class Forall(pred: T => Boolean, protected val pit: IterableSplitter[T @uncheckedVariance])
  extends Accessor[Boolean, Forall] {
    ...
    protected def newSubtask(p: IterableSplitter[T @uncheckedVariance]) = new Forall(pred, p)
    override def merge(that: Forall) = result = result && that.result
  }

  protected class Exists(pred: T => Boolean, protected val pit: IterableSplitter[T @uncheckedVariance])
  extends Accessor[Boolean, Exists] {
    ...
    protected def newSubtask(p: IterableSplitter[T @uncheckedVariance]) = new Exists(pred, p)
    override def merge(that: Exists) = result = result || that.result
  }

  protected class Find[U >: T](pred: T => Boolean, protected val pit: IterableSplitter[T @uncheckedVariance])
  extends Accessor[Option[U], Find[U]] {
    ...
    protected def newSubtask(p: IterableSplitter[T @uncheckedVariance]) = new Find(pred, p)
    override def merge(that: Find[U]) = if (this.result == None) result = that.result
  }

  protected class Filter[U >: T, This >: Repr](pred: T => Boolean, cbf: CombinerFactory[U, This], protected val pit: IterableSplitter[T @uncheckedVariance])
  extends Transformer[Combiner[U, This], Filter[U, This]] {
    ...
    protected def newSubtask(p: IterableSplitter[T @uncheckedVariance]) = new Filter(pred, cbf, p)
    override def merge(that: Filter[U, This]) = result = result `combine` that.result
  }

  protected class FilterNot[U >: T, This >: Repr](pred: T => Boolean, cbf: CombinerFactory[U, This], protected val pit: IterableSplitter[T @uncheckedVariance])
  extends Transformer[Combiner[U, This], FilterNot[U, This]] {
    @volatile var result: Combiner[U, This] = null
    def leaf(prev: Option[Combiner[U, This]]) = {
      result = pit.filterNot2combiner(pred, reuse(prev, cbf()))
    }
    protected def newSubtask(p: IterableSplitter[T @uncheckedVariance]) = new FilterNot(pred, cbf, p)
    override def merge(that: FilterNot[U, This]) = result = result `combine` that.result
  }

  protected class Copy[U >: T, That](cfactory: CombinerFactory[U, That], protected val pit: IterableSplitter[T @uncheckedVariance])
  extends Transformer[Combiner[U, That], Copy[U, That]] {
    ...
    protected def newSubtask(p: IterableSplitter[T @uncheckedVariance]) = new Copy[U, That](cfactory, p)
    override def merge(that: Copy[U, That]) = result = result `combine` that.result
  }

  protected class Partition[U >: T, This >: Repr]
  (pred: T => Boolean, cbfTrue: CombinerFactory[U, This], cbfFalse: CombinerFactory[U, This], protected val pit: IterableSplitter[T @uncheckedVariance])
  extends Transformer[(Combiner[U, This], Combiner[U, This]), Partition[U, This]] {
    ...
    protected def newSubtask(p: IterableSplitter[T @uncheckedVariance]) = new Partition(pred, cbfTrue, cbfFalse, p)
    override def merge(that: Partition[U, This]) = result = (result._1 `combine` that.result._1, result._2 `combine` that.result._2)
  }

  protected class GroupBy[K, U >: T](
    f: U => K,
    mcf: () => HashMapCombiner[K, U],
    protected val pit: IterableSplitter[T @uncheckedVariance]
    ...
    protected def newSubtask(p: IterableSplitter[T @uncheckedVariance]) = new GroupBy(f, mcf, p)
  ...
  protected class Take[U >: T, This >: Repr]
  (n: Int, cbf: CombinerFactory[U, This], protected val pit: IterableSplitter[T @uncheckedVariance])
  extends Transformer[Combiner[U, This], Take[U, This]] {
    @volatile var result: Combiner[U, This] = null
    def leaf(prev: Option[Combiner[U, This]]) = {
      result = pit.take2combiner(n, reuse(prev, cbf()))
    }
    protected def newSubtask(p: IterableSplitter[T @uncheckedVariance]) = throw new UnsupportedOperationException
    override def split = {
  ...
  protected class Drop[U >: T, This >: Repr]
  (n: Int, cbf: CombinerFactory[U, This], protected val pit: IterableSplitter[T @uncheckedVariance])
  ...
  protected class Slice[U >: T, This >: Repr]
  (from: Int, until: Int, cbf: CombinerFactory[U, This], protected val pit: IterableSplitter[T @uncheckedVariance])
  ...
  protected class SplitAt[U >: T, This >: Repr]
  (at: Int, cbfBefore: CombinerFactory[U, This], cbfAfter: CombinerFactory[U, This], protected val pit: IterableSplitter[T  @uncheckedVariance])
  ...
  protected class TakeWhile[U >: T, This >: Repr]
  (pos: Int, pred: T => Boolean, cbf: CombinerFactory[U, This], protected val pit: IterableSplitter[T @uncheckedVariance])
  ...
  protected class Span[U >: T, This >: Repr]
  (pos: Int, pred: T => Boolean, cbfBefore: CombinerFactory[U, This], cbfAfter: CombinerFactory[U, This], protected val pit: IterableSplitter[T @uncheckedVariance])
  ...
  protected class Zip[U >: T, S, That](pbf: CombinerFactory[(U, S), That], protected val pit: IterableSplitter[T @uncheckedVariance], val othpit: SeqSplitter[S])
  ...
  protected class ZipAll[U >: T, S, That]
  (len: Int, thiselem: U, thatelem: S, pbf: CombinerFactory[(U, S), That], protected val pit: IterableSplitter[T @uncheckedVariance], val othpit: SeqSplitter[S])
  ...
  protected class CopyToArray[U >: T, This >: Repr](from: Int, len: Int, array: Array[U], protected val pit: IterableSplitter[T @uncheckedVariance])
  ...
  protected class ToParCollection[U >: T, That](cbf: CombinerFactory[U, That], protected val pit: IterableSplitter[T @uncheckedVariance])
  extends Transformer[Combiner[U, That], ToParCollection[U, That]] {
    ...
    protected def newSubtask(p: IterableSplitter[T @uncheckedVariance]) = new ToParCollection[U, That](cbf, p)
    override def merge(that: ToParCollection[U, That]) = result = result `combine` that.result
  }

  protected class ToParMap[K, V, That](cbf: CombinerFactory[(K, V), That], protected val pit: IterableSplitter[T @uncheckedVariance])(implicit ev: T <:< (K, V))
  ...
  protected class CreateScanTree[U >: T](from: Int, len: Int, z: U, op: (U, U) => U, protected val pit: IterableSplitter[T @uncheckedVariance])
  extends Transformer[ScanTree[U], CreateScanTree[U]] {
    ...
    protected def newSubtask(pit: IterableSplitter[T @uncheckedVariance]) = throw new UnsupportedOperationException
    override def split = {
      val pits = pit.splitWithSignalling
  ...
  protected class FromScanTree[U >: T, That]
  ...
  protected def scanBlockSize = (thresholdFromSize(size, tasksupport.parallelismLevel) / 2) max 1

  protected sealed trait ScanTree[U >: T] {
    def beginsAt: Int
  ...
  protected case class ScanNode[U >: T](left: ScanTree[U], right: ScanTree[U]) extends ScanTree[U] {
  ...
  protected case class ScanLeaf[U >: T]
