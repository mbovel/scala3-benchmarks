...
extends ParIterableLike[T, CC, Repr, Sequential]
  ...
  protected type SuperParIterator = IterableSplitter[T @uncheckedVariance]
  ...
  protected def down(p: IterableSplitter[?]) = p.asInstanceOf[SeqSplitter[T]]

  protected trait ParSeqLikeAccessor[R, Tp] extends Accessor[R, Tp] {
    protected val pit: SeqSplitter[T]
  }

  protected trait ParSeqLikeTransformer[R, Tp] extends ParSeqLikeAccessor[R, Tp] with Transformer[R, Tp]

  protected class SegmentLength(pred: T => Boolean, from: Int, protected val pit: SeqSplitter[T @uncheckedVariance])
  extends ParSeqLikeAccessor[(Int, Boolean), SegmentLength] {
    ...
    protected def newSubtask(p: SuperParIterator) = throw new UnsupportedOperationException
    override def split = {
  ...
  protected class IndexWhere(pred: T => Boolean, from: Int, protected val pit: SeqSplitter[T @uncheckedVariance])
  ...
  protected class LastIndexWhere(pred: T => Boolean, pos: Int, protected val pit: SeqSplitter[T @uncheckedVariance])
  ...
  protected class Reverse[U >: T, This >: Repr](cbf: () => Combiner[U, This], protected val pit: SeqSplitter[T @uncheckedVariance])
  extends ParSeqLikeTransformer[Combiner[U, This], Reverse[U, This]] {
    ...
    protected def newSubtask(p: SuperParIterator) = new Reverse(cbf, down(p))
    override def merge(that: Reverse[U, This]) = result = that.result `combine` result
  }

  protected class ReverseMap[S, That](f: T => S, pbf: () => Combiner[S, That], protected val pit: SeqSplitter[T @uncheckedVariance])
  extends ParSeqLikeTransformer[Combiner[S, That], ReverseMap[S, That]] {
    ...
    protected def newSubtask(p: SuperParIterator) = new ReverseMap(f, pbf, down(p))
    override def merge(that: ReverseMap[S, That]) = result = that.result `combine` result
  }

  protected class SameElements[U >: T](protected val pit: SeqSplitter[T @uncheckedVariance], val otherpit: SeqSplitter[U @uncheckedVariance])
  ...
  protected class Updated[U >: T, That](pos: Int, elem: U, pbf: CombinerFactory[U, That], protected val pit: SeqSplitter[T @uncheckedVariance])
  ...
  protected class ParSeqLikeZip[U >: T, S, That](len: Int, cf: CombinerFactory[(U, S), That], protected val pit: SeqSplitter[T @uncheckedVariance], val otherpit: SeqSplitter[S])
  ...
  protected class Corresponds[S](corr: (T, S) => Boolean, protected val pit: SeqSplitter[T @uncheckedVariance], val otherpit: SeqSplitter[S])
