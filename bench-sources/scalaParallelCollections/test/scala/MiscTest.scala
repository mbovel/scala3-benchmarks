...
//    assertEquals("List(List(1, 2))", s.transpose.map(_.toList).toList.toString)
  ...
  def si6467: Unit = {
    ...
    assertEquals(Seq(0 until 100*).foldLeft(new java.lang.StringBuffer)(_ `append` _).toString, (0 until 100).mkString)
    assertEquals(Seq(0 until 100*).par.aggregate(new java.lang.StringBuffer)(_ `append` _, _ `append` _).toString, (0 until 100).mkString)
  }
  ...
  def si8955: Unit = {
    def test(): Unit =
      scala.collection.parallel.immutable.ParSet[Int]((1 to 10000)*) `foreach` (x => ()) // hangs non deterministically
