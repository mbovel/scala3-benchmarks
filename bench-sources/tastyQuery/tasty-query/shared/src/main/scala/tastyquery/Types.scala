...
import scala.annotation.{constructorOnly, nowarn, tailrec, targetName}
...
object Types {
  ...
  sealed trait LambdaType extends TypeOrMethodic with ParamRefBinder {
    ...
    val paramRefs: List[ParamRefType] =
      List.tabulate(paramNames.size)((newParamRef(_): @unchecked): @nowarn("msg=transitively initialized"))
