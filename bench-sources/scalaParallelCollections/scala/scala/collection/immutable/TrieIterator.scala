...
private[collection] abstract class TrieIterator[+T](elems: Array[Iterable[T]]) extends collection.Iterator[T] {
  ...
  private var depth                                     = initDepth
  private val arrayStack: Array[Array[Iterable[T @uV]]] = initArrayStack
  private var posStack                                  = initPosStack
  private var arrayD: Array[Iterable[T @uV]]            = initArrayD
  private var posD                                      = initPosD
  private var subIter                                   = initSubIter

  private def getElems(x: Iterable[T]): Array[Iterable[T]] = ((x: @unchecked) match {
    case x: HashTrieMap[?, ?] => x.elems
    case x: HashTrieSet[?]    => x.elems
  }).asInstanceOf[Array[Iterable[T]]]

  private def collisionToArray(x: Iterable[T]): Array[Iterable[T]] = ((x: @unchecked) match {
  ...
  private def newIterator(xs: Array[Iterable[T]]) = new TrieIterator(xs) {
    final override def getElem(x: AnyRef): T = outer.getElem(x)
  }

  private def iteratorWithSize(arr: Array[Iterable[T]]): (Iterator[T], Int) =
    (newIterator(arr), ((arr.map(_.size): Array[Int]): scala.collection.IterableOps[Int, scala.collection.Iterable, ?]).sum)

  private def arrayToIterators(arr: Array[Iterable[T]]): SplitIterators = {
    val (fst, snd) = arr.splitAt(arr.length / 2)

    (iteratorWithSize(snd), newIterator(fst))
  }
  private def splitArray(ad: Array[Iterable[T]]): SplitIterators =
  ...
  @tailrec private def next0(elems: Array[Iterable[T]], i: Int): T = {
