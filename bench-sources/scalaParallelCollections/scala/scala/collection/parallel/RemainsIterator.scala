...
self =>
  ...
  class Taken(taken: Int) extends IterableSplitter[T] {
    ...
    protected def takeSeq[PI <: IterableSplitter[T @uncheckedVariance]](sq: Seq[PI])(taker: (PI, Int) => PI) = {
...
self =>
  ...
  class RemainsIteratorTaken(tk: Int) extends Taken(tk) with SeqSplitter[T] {
    ...
    def psplit(sizes: Int*): Seq[SeqSplitter[T]] = takeSeq(self.psplit(sizes*)) { (p, n) => p.take(n) }
  }
  ...
  class RemainsIteratorMapped[S](f: T => S) extends Mapped[S](f) with SeqSplitter[S] {
    ...
    def psplit(sizes: Int*): Seq[SeqSplitter[S]] = self.psplit(sizes*).map {_ `map` f }
  }
  ...
  class RemainsIteratorAppended[U >: T, PI <: SeqSplitter[U]](it: PI) extends Appended[U, PI](it) with SeqSplitter[U] {
    ...
    def psplit(sizes: Int*): Seq[SeqSplitter[U]] = if (firstNonEmpty) {
      ...
      val thats = that.psplit(thatsizes*)
    ...
    } else curr.asInstanceOf[SeqSplitter[U]].psplit(sizes*)
  }
  ...
  class Patched[U >: T](from: Int, patch: SeqSplitter[U], replaced: Int) extends SeqSplitter[U] {
    ...
    def psplit(sizes: Int*) = trio.psplit(sizes*)
  }
